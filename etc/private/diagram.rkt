...
(define (contracted p #:hole-color [hole-color #f])
  (define rectangle-fun
    (if hole-color (filled-rectangle-holes hole-color) filled-rectangle))
  (define contracts
    (rectangle-fun CONTRACT-WIDTH (pict-height p) #:color CONTRACT-COLOR #:draw-border? #f))
  (hc-append contracts p))

(define (label txt orient)
  (text txt LABEL-FONT LABEL-SIZE (if (eq? orient 'lr) 0 (/ pi 2)))
  #;(vc-append
...
(define (arrow p first-tag second-tag text
  ...
  (pin-arrow-line
   ...
   (if (eq? orient 'lr) rc-find cb-find)
   (find-tag p second-tag)
   (if (eq? orient 'lr) lc-find ct-find)
   #:under? #t
   #:style style
   #:line-width
   (if (eq? orient 'lr) 1 LINE-WIDTH-CONVERT)
   #:label (or dl (label text orient))
   #:x-adjust-label (if (eq? orient 'lr) 0 adjust)))

(define base
  (blank STAGE-WIDTH STAGE-HEIGHT))

(define (diagrams _1 _2 _3)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define ut-require-ty
  (build-path figures-dir "ut-require-ty"))

(save-pict* ut-require-ty
(let ()
  (define main
...
;;   #:go (coord 1/2 0.345 'cc) (colorize (hline STAGE-WIDTH 1) "gray")

   #:go (coord 1/4 1/2 'cc) (tag-pict pre-main 'pre-main)
   #:go (coord 3/4 1/2 'cc) (tag-pict pre-streams 'pre-streams)

  ;; #:go (coord 1/2 0.645 'cc) (colorize (hline STAGE-WIDTH 1) "gray")
   ...
   #:set (arrow ppict-do-state 'pre-main 'final-main "unchanged" #:style 'long-dash #:orient 'ud #:adjust -65)

   #:set (arrow ppict-do-state 'src-streams 'pre-streams "explicate" #:orient 'ud #:adjust 30)
   #:set (arrow ppict-do-state 'pre-streams 'final-streams "" #:orient 'ud #:adjust -70 #:different-label ae)
   )))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define ty-require-ut
  (build-path figures-dir "ty-require-ut"))

(save-pict* ty-require-ut
(let ()
  (define main
...
;;   #:go (coord 1/2 0.345 'cc) (colorize (hline STAGE-WIDTH 1) "gray")
   ...
   ;;#:go (coord 1/20 0.4 'lt) (annotate "to analyze")
 ...
 ;;  #:go (coord 1/2 0.645 'cc) (colorize (hline STAGE-WIDTH 1) "gray")
   ...
   #:go (coord 1/20 0.7 'lt) (annotate "output")
   #:go (coord 1/4 4/5 'cc) (tag-pict after-main 'final-main)
   ...
   #:set (arrow ppict-do-state 'pre-main 'final-main "analyze" #:orient 'ud #:adjust -53)
   #:set (arrow ppict-do-state 'pre-proxy 'final-proxy "erase" #:orient 'ud #:adjust -42)
